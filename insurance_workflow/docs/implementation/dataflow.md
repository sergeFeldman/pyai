...
| 8 | `rules.RuleRegistry.execute("claim_appeal", context, entities=...)` | Walks the DAG in topological generation order; prunes any rule whose required internal inputs were never produced by upstream rules; evaluates rules that pass the pruning check and writes their outputs to the shared context; records every rule visited with its outcome (triggered / skipped_no_match / pruned / skipped_precondition); returns `RuleExecutionResult` |
